    ...
    Array::Item: Copy,
{
    ...
    let ptr = MutPtr(ret.mut_ptr());
    (0..Array::len()).into_par_iter().for_each(|i| unsafe {
        std::ptr::write(ptr.add(i), initializer(i));
    });
    ret
}
...
where
    I: IndexedParallelIterator<Item = Array::Item>,
    Array: IsParArray,
    Array::Item: Copy + Send + Sync,
{
    ...
    let ptr = MutPtr(ret.mut_ptr());
    iter.enumerate()
        .for_each(|(indx, item)| unsafe { std::ptr::write(ptr.add(indx), item) });
    Some(ret)
}

pub unsafe trait IsParArray: IsArray {
    fn mut_ptr(&mut self) -> *mut Self::Item;
}

macro_rules! impl_is_par_array {
    ($($size:expr)+) => ($(
        unsafe impl<T> IsParArray for [T; $size] {
            fn mut_ptr(&mut self) -> *mut Self::Item {
                self.as_mut_ptr()
            }
        }
...
mod test {
    ...
    #[test]
    fn test_intializing_array_in_parallel_works() {
